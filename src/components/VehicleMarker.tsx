import "./MapWithMarkers/MapWithMarkers.scss";
...
export default function VehicleMarker(props: VehicleType) {
  ...
  return (
    <>
      <MarkerPopup show={isPopupVisible} onClickOutside={hidePopup}>
        <div className={"marker-popup-wrapper"}>
          <div><span>Name: </span>{props.properties.name}</div>
          <div><span>Range: </span>{`${props.properties.rangeKm}km`}</div>
          <div><span>Status: </span>{props.properties.status}</div>
          <div><span>Battery Level: </span>{`${props.properties.batteryLevelPct}%`}</div>
          <div><span>Plate number: </span>{props.properties.plateNumber}</div>
        </div>
      </MarkerPopup>
      <svg
          onClick={showPopup}
      ...
      >
        ...
        <g
          ...
          fill={props.properties.status === "available" ? "#228B22" : "#DC143C"}
          stroke="none"
        >
